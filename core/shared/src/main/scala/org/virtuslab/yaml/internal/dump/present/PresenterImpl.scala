...
object PresenterImpl extends Presenter {
  override def asString(events: Seq[EventKind]): String = {
    ...
    def requiresDoubleQuoting(s: String): Boolean = {
      ...
      var c = s.charAt(0)
      if ((c == '-' || c == '?' || c == ':') && (len == 1 || Character.isWhitespace(s.charAt(1))))
        return true
      if (
      ...
      while (i < len) {
        ...
        if (prev == ':' && c == ' ' || prev == ' ' && c == '#') return true
        prev = c
      }
      (c: @switch) match {
        case 'n' | 'N' | '~' =>
          Tag.nullPattern.pattern.matcher(s).matches
        case 'f' | 'F' =>
          Tag.falsePattern.pattern.matcher(s).matches
        case 't' | 'T' =>
          Tag.truePattern.pattern.matcher(s).matches
        case '-' =>
